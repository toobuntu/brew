...
### Keeping CI workflows up to date

The `.github/workflows` files generated by `brew tap-new` may fall behind upstream Homebrew as CI infrastructure evolves. Use `brew sync-tap-workflows` to fetch and adapt the latest workflow files from upstream:

```console
# For formula taps (fetches autobump.yml, actionlint.yml, cache.yml, tests.yml, publish.yml):
$ brew sync-tap-workflows --formula $YOUR_GITHUB_USERNAME/homebrew-tap

# For cask taps (fetches ci.yml, ci-retry.yml, actionlint.yml, autobump.yml, cache.yml):
$ brew sync-tap-workflows --cask $YOUR_GITHUB_USERNAME/homebrew-cask
```

Pass `--bot-username` and `--bot-email` to configure the automated bump bot credentials in `autobump.yml`. Without them, the file is written as `autobump.yml.disabled` with a `TODO` comment.

Use `--dry-run` (`-n`) to preview what would be written without making any changes.
